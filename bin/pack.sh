#!/usr/bin/env bash
#
# Build a release ZIP of the 404 to 301 - Logs Exporter addon.
#
# Steps:
...
SLUG="404-to-301-logs-exporter"
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PLUGIN_FILE="$ROOT/404-to-301-logs-exporter.php"
...
wp i18n make-pot "$ROOT" "$ROOT/languages/$SLUG.pot" \
	--slug="$SLUG" \
	--domain="$SLUG" \
	--exclude="node_modules,build,releases,bin"
